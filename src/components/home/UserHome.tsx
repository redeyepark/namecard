 ...
 *
 * When `embedded={true}`, renders only the content sections (profile, quick actions,
 * card grid) without the page wrapper and header, suitable for integration into
 * other pages like LandingPage.
 */
export function UserHome({ embedded = false }: UserHomeProps) {
  const { user } = useAuth();
  ...
  // --- Embedded mode: compact inline states ---
  if (embedded) {
    if (loading) {
      return (
        <div className="flex justify-center py-16">
          <svg
            className="animate-spin h-6 w-6 text-[#020912]/30"
