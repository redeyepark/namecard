...
- 관리자 질문 관리: 관리자가 커뮤니티 질문을 직접 생성, 수정, 삭제할 수 있는 기능
  - API: GET/POST `/api/admin/questions`, PUT/DELETE `/api/admin/questions/[id]`
...
### Link-in-Bio 프로필 리디자인 (SPEC-LINKBIO-001)

- `/profile/[id]` 프로필 페이지를 Link-in-Bio 스타일로 완전 리디자인
- 상단 프로필 영역: 아바타 이미지, 표시 이름, 자기소개
- 소셜 아이콘 행: Instagram, Facebook, LinkedIn, Email, GitHub, YouTube, Twitter, Website 플랫폼 아이콘 배열 (SocialIconRow 컴포넌트)
- 커스텀 링크 목록: 사용자 정의 링크 버튼 나열 (LinkButton, LinkList 컴포넌트)
- 포트폴리오 카드 갤러리: 사용자의 명함 카드 목록 (CardPortfolio 컴포넌트)
- 링크 관리: 추가, 수정, 삭제, 드래그 순서 변경 (LinkEditor, LinkEditModal, SocialLinksEditor 컴포넌트)
- 대시보드 설정에서 프로필 편집: 아바타 업로드, 소셜 링크, 커스텀 링크 관리
- DB 테이블: `profiles` (사용자 프로필), `profile_links` (커스텀 링크, sort_order 지원)
- API 라우트:
  - `/api/profiles/[id]` (GET 프로필 조회)
  - `/api/profiles/[id]/cards` (GET 사용자 카드 목록)
  - `/api/profiles/[id]/links` (GET 공개 링크 목록)
  - `/api/profiles/me` (GET/PUT 내 프로필 조회/수정)
  - `/api/profiles/me/avatar` (POST 아바타 이미지 업로드)
  - `/api/profiles/me/links` (GET/POST 내 링크 목록/생성)
  - `/api/profiles/me/links/[linkId]` (PUT/DELETE 링크 수정/삭제)
  - `/api/profiles/me/links/reorder` (PUT 링크 순서 변경)
- 훅: useLinks (src/hooks/useLinks.ts)
...
### 질문 & 생각 공유 (SPEC-COMMUNITY-003)

- `/community/questions` 경로에서 질문 피드 조회 (해시태그 필터, 커서 기반 페이지네이션)
- `/community/questions/[id]` 경로에서 질문 상세 및 생각(답변) 목록 조회
- 질문 CRUD: 인증 사용자가 질문 작성, 본인 질문 수정/삭제
- 생각(Thought) 시스템: 질문에 대한 답변 작성, 생각에 좋아요 토글
- 해시태그 기반 질문 분류 및 검색
- API 라우트: `/api/questions` (GET/POST), `/api/questions/[id]` (GET/DELETE), `/api/questions/[id]/thoughts` (GET/POST), `/api/questions/[id]/thoughts/[thoughtId]` (DELETE), `/api/thoughts/[id]/like` (POST/DELETE)
- DB 테이블: `community_questions` (질문), `community_thoughts` (생각/답변), `thought_likes` (생각 좋아요)
- 비정규화 카운터: `community_questions.thought_count`, `community_thoughts.like_count` (트리거 기반 자동 업데이트)
- RLS 정책: 활성 질문/생각 공개 열람, 본인 작성/수정/삭제, 인증 사용자 좋아요
- 컴포넌트: QuestionCard, QuestionFeed, QuestionForm, QuestionDetail, QuestionFilters, ThoughtCard, ThoughtForm, ThoughtList, ThoughtLikeButton, HashtagChip (src/components/community/)
- 훅: useQuestions, useThoughts, useThoughtLike (src/hooks/)
- 스토리지: question-storage.ts (XSS 방지 `stripHtml()` 유틸 포함)
- 생각(Thought) RLS 정책 수정: 생각 등록 불가 버그 수정 (migration 015_fix_thoughts_rls.sql)

### 커피챗 요청 & 수락 (SPEC-COMMUNITY-004)

- `/community/coffee-chat` 경로에서 회원 탐색 및 커피챗 요청
- `/community/coffee-chat/my` 경로에서 내 커피챗 요청/수신 목록 관리
- 5-상태 FSM: pending (대기중) -> accepted (수락됨) / declined (거절됨) / cancelled (취소됨) -> completed (완료됨)
- 회원 탐색: 프로필 공개 설정된 회원 목록에서 커피챗 요청 (discoverable members)
- 커서 기반 페이지네이션: 회원 목록 및 커피챗 목록 무한 스크롤
- Rate Limiting: 24시간 내 최대 5건 요청 제한
- 양방향 중복 방지: LEAST/GREATEST partial unique index로 A->B, B->A 동시 활성 요청 차단
- 조건부 이메일 공개: 수락(accepted) 시에만 상대방 이메일 노출
- Pending 카운트 배지: 60초 간격 폴링으로 실시간 수신 대기 건수 표시
- 미팅 선호도: 온라인/오프라인/상관없음 3가지 옵션
- API 라우트: `/api/coffee-chat` (GET/POST), `/api/coffee-chat/[id]` (GET), `/api/coffee-chat/[id]/respond` (POST), `/api/coffee-chat/pending-count` (GET), `/api/members/discoverable` (GET)
- DB 테이블: `coffee_chat_requests` (5-상태 FSM, 자기 자신 요청 방지 CHECK, LEAST/GREATEST 부분 유니크 인덱스)
- RLS 정책: 당사자만 조회, 요청자만 생성, 당사자만 상태 변경
- 컴포넌트: CoffeeChatCard, CoffeeChatDetail, CoffeeChatList, CoffeeChatButton, CoffeeChatRequestModal, CoffeeChatActions, CoffeeChatStatusBadge, CoffeeChatBadge, MemberCard, MemberDiscoverGrid (src/components/coffee-chat/)
- 훅: useCoffeeChat, useCoffeeChatCount (src/hooks/)
- 스토리지: coffee-chat-storage.ts (상태 전이 검증, 사용자 프로필 일괄 조회)
- 커피챗 개인 선호도 설정: 커피챗 프로필에 개인 선호도 추가 (migration 013_add_coffee_chat_preferences.sql)

### 커뮤니티 네비게이션

- CommunityNav 탭 네비게이션 컴포넌트 (src/components/community/CommunityNav.tsx)
- 질문/생각 탭, 커피챗 탭, 설문/투표 탭 간 전환
- CoffeeChatBadge 통합: pending 수신 건수를 커피챗 탭에 실시간 배지 표시

### MBTI 진단 시스템

- 48개 MBTI 질문 (차원당 12개: EI, SN, TF, JP)
- 이진 선택 (A/B) 기반 간편 답변
- 순차적 잠금 해제: 이전 질문을 답해야 다음 질문 활성화
- 레벨 시스템: Lv.1 (0개), Lv.2 (12+), Lv.3 (24+), Lv.4 (36+), Lv.5 (48개)
- 모든 질문 완료 시 MBTI 유형 자동 판정 (차원별 다수결, 동률 시 첫 번째 문자)
- `user_profiles` 테이블에 `level`, `mbti_type` 컬럼 연동
- 커뮤니티 질문 페이지 상단에 MbtiSection 표시
- 대시보드와 프로필 페이지에 레벨 배지(MbtiLevelBadge) 및 MBTI 결과 배지(MbtiResultBadge) 표시
- 컴포넌트: MbtiSection, MbtiQuestionCard, MbtiLevelBadge, MbtiResultBadge (src/components/mbti/)
- 훅: useMbtiProgress (src/hooks/useMbtiProgress.ts)
- 스토리지: mbti-storage.ts (src/lib/mbti-storage.ts)
- 타입: mbti.ts (src/types/mbti.ts)
- API: GET `/api/mbti/questions` (진행 상황), POST `/api/mbti/answer` (답변 제출)
- DB 테이블: `mbti_questions`, `mbti_answers` (016_add_mbti_system.sql, 017_add_more_mbti_questions.sql)

### 커뮤니티 설문/투표 시스템 (SPEC-SURVEY-001)

- `/community/surveys` 경로에서 설문 피드 조회
- `/community/surveys/[id]` 경로에서 설문 상세 및 투표 결과 조회
- 설문 CRUD: 인증 사용자가 설문 작성 (제목, 설명, 선택지, 마감일, 복수선택, 익명투표 옵션)
- 투표 시스템: 설문 선택지에 투표, 실시간 결과 표시
- 관리자 공식 설문 배지(OfficialBadge)
- 해시태그 기반 설문 분류 및 필터링
- API: GET/POST `/api/surveys`, GET `/api/surveys/[id]`, POST `/api/surveys/[id]/vote`
- DB 테이블: `community_surveys`, `survey_options`, `survey_votes` (014_add_surveys.sql)
- 컴포넌트: SurveyCard, SurveyFeed, SurveyForm, SurveyDetail, SurveyVoteUI, SurveyResults, SurveyFilters, OfficialBadge (src/components/survey/)
- 훅: useSurveys, useSurveyDetail, useSurveyVote, useSurveyCreate (src/hooks/)
- 스토리지: survey-storage.ts (src/lib/survey-storage.ts)
- 타입: survey.ts (src/types/survey.ts)
...
| `/community/questions` | 공개 (작성은 인증 필요) | 질문 피드 |
| `/community/questions/[id]` | 공개 (생각 작성은 인증 필요) | 질문 상세 + 생각 목록 |
| `/community/surveys` | 공개 (작성은 인증 필요) | 설문/투표 피드 |
| `/community/surveys/[id]` | 공개 (투표는 인증 필요) | 설문 상세 + 투표 결과 |
| `/community/coffee-chat` | 인증 필요 | 회원 탐색 + 커피챗 요청 |
| `/community/coffee-chat/my` | 인증 필요 | 내 커피챗 요청/수신 목록 |
